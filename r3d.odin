...
*/
Flag :: enum c.uint {
	NONE, /**< No special rendering flags */
	FXAA = 10, /**< Enables Fast Approximate Anti-Aliasing (FXAA) */
	Blit_Linear = 11, /**< Uses linear filtering when blitting the final image */
	Aspect_Keep = 12, /**< Maintains the aspect ratio of the internal resolution when blitting the final image */
	Stencil_Test = 13, /**< Performs a stencil test on each rendering pass affecting geometry */
	Eight_Bit_Normals = 14, /**< Use 8-bit precision for the normals buffer (deferred); default is 16-bit float */
	No_Frustum_Culling = 15, /**< Disables internal frustum culling. Manual culling is allowed, but may break shadow visibility if objects casting shadows are skipped. */
	Transparent_Sroting = 16, /**< Back-to-front sorting of transparent objects for correct blending of non-discarded fragments. Be careful, in 'force forward' mode this flag will also sort opaque objects in 'near-to-far' but in the same sorting pass. */
	Opaque_Sorting = 17, /**< Front-to-back sorting of opaque objects to optimize depth testing at the cost of additional sorting. Please note, in 'force forward' mode this flag has no effect, see transparent sorting. */
...
*
* This type is used by `Mesh` and `Sprite` objects to indicate
* which rendering layer(s) they belong to. Active layers are controlled
* globally via the functions:
*
* - void EnableLayers(Layer bitfield);
* - void DisableLayers(Layer bitfield);
*
...
BlendMode :: enum c.uint {
	...
	ALPHA               = 1, ///< Alpha blending: source color is blended with the destination based on alpha value.
	ADDITIVE            = 2, ///< Additive blending: source color is added to the destination, making bright effects.
	MULTIPLY            = 3, ///< Multiply blending: source color is multiplied with the destination, darkening the image.
	PREMULTIPLIED_ALPHA = 4, ///< Premultiplied alpha blending: source color is blended with the destination assuming the source color is already multiplied by its alpha.
}
...
DepthMode :: enum c.uint {
	READ_WRITE = 0, ///< Enable depth testing and writing to the depth buffer.
	READ_ONLY  = 1, ///< Enable depth testing but disable writing to the depth buffer.
	DISABLED   = 2, ///< Disable depth testing and writing to the depth buffer.
}
...
*/
Bloom :: enum c.uint {
	DISABLED = 0, ///< Bloom effect is disabled. The scene is rendered without any glow enhancement.
	MIX      = 1, ///< Blends the bloom effect with the original scene using linear interpolation (Lerp).
	ADDITIVE = 2, ///< Adds the bloom effect additively to the scene, intensifying bright regions.
	SCREEN   = 3, ///< Combines the scene and bloom using screen blending, which brightens highlights
}
...
*/
Fog :: enum c.uint {
	DISABLED = 0, ///< Fog effect is disabled.
	LINEAR   = 1, ///< Fog density increases linearly with distance from the camera.
	EXP2     = 2, ///< Exponential fog (exp2), where density increases exponentially with distance.
	EXP      = 3, ///< Exponential fog, similar to EXP2 but with a different rate of increase.
}
...
*/
Tonemap :: enum c.uint {
	LINEAR   = 0, ///< Simple linear mapping of HDR values.
	REINHARD = 1, ///< Reinhard tone mapping, a balanced method for compressing HDR values.
	FILMIC   = 2, ///< Filmic tone mapping, mimicking the response of photographic film.
	ACES     = 3, ///< ACES tone mapping, a high-quality cinematic rendering technique.
	AGX      = 4, ///< AGX tone mapping, a modern technique designed to preserve both highlight and shadow details for HDR rendering.
	COUNT    = 5, ///< Number of tone mapping modes (used internally)
}
...
*/
Dof :: enum c.uint {
	DISABLED = 0, ///< Depth of field effect is disabled.
	ENABLED  = 1, ///< Depth of field effect is enabled.
}
...
*/
AnimMode :: enum c.uint {
	INTERNAL = 0, ///< Default animation solution
	CUSTOM   = 1, ///< User supplied matrices via Model::boneOverride 
}
...
Material :: struct {
	...
	uvScale:       c.int, /**< UV scale factor applied to the texture coordinates.
                                           ...
                                           *  For sprites, this value is overridden automatically.
                                           */
	alphaCutoff:   c.float, /**< Alpha threshold below which fragments are discarded. */
}
...
*/
Keyframe :: struct {
	time:  c.float, ///< Normalized time of the keyframe, ranging from 0.0 to 1.0.
	value: c.float, ///< The value of the interpolation at this keyframe.
}
...
ParticleSystem :: struct {
	...
	emissionRate:                c.float, ///< The rate of particle emission in particles per second. Default: 10.0f.
	spreadAngle:                 c.float, ///< The angle of propagation of the particles in a cone (degrees). Default: 0.0f.
	scaleOverLifetime:           ^InterpolationCurve, ///< Curve controlling the scale evolution of the particles over their lifetime. Default: NULL.
...
foreign lib {
	...
	* @param flags Flags indicating internal behavior (modifiable via SetState).
	*/
	Init :: proc(resWidth: c.int, resHeight: c.int, flags: Flag) ---
	...
	*/
	HasState :: proc(flag: Flag) -> c.bool ---
	...
	* state flags. Flags can be later cleared with ClearState.
	*
	* @param flags The flags to set.
	*/
	SetState :: proc(flag: Flag) ---
	...
	*/
	GetResolution :: proc(width: ^c.int, height: ^c.int) ---
	...
	*/
	UpdateResolution :: proc(width: c.int, height: c.int) ---
	...
	*/
	SetSceneBounds :: proc(sceneBounds: rl.BoundingBox) ---
	...
	* activated.
	*
	* @return Layer Bitfield of active layers.
	*/
	...
	*/
	SetActiveLayers :: proc(layers: Layer) ---
	...
	* directly to the screen framebuffer (same behavior as Begin).
	*
	* @param camera The camera to use for rendering the scene.
	* @param target Optional pointer to a RenderTexture to render into. Can be NULL to render
	*               directly to the screen.
	*/
	BeginEx :: proc(camera: rl.Camera3D, target: ^rl.RenderTexture) ---
	...
	*/
	DrawMesh :: proc(mesh: ^Mesh, material: ^Material, transform: rl.Matrix) ---
	...
	*/
	DrawMeshInstanced :: proc(mesh: ^Mesh, material: ^Material, instanceTransforms: [^]rl.Matrix, instanceCount: c.int) ---
	...
	* @param instanceCount The number of instances to render. Must be greater than 0.
	*/
	DrawMeshInstancedEx :: proc(mesh: ^Mesh, material: ^Material, instanceTransforms: [^]rl.Matrix, instanceColors: [^]rl.Color, instanceCount: c.int) ---
	...
	* @param instanceCount The number of instances to render. Must be greater than 0.
	*/
	DrawMeshInstancedPro :: proc(mesh: ^Mesh, material: ^Material, globalAabb: ^rl.BoundingBox, globalTransform: rl.Matrix, instanceTransforms: [^]rl.Matrix, transformsStride: c.int, instanceColors: [^]rl.Color, colorsStride: c.int, instanceCount: c.int) ---
	...
	*/
	DrawModel :: proc(model: ^Model, position: rl.Vector3, scale: c.float) ---
	...
	* @param scale The scale factor to apply to the model.
	*/
	DrawModelEx :: proc(model: ^Model, position: rl.Vector3, rotationAxis: rl.Vector3, rotationAngle: c.float, scale: rl.Vector3) ---
	...
	* @param instanceCount The number of instances to render. Must be greater than 0.
	*/
	DrawModelInstanced :: proc(model: ^Model, instanceTransforms: [^]rl.Matrix, instanceCount: c.int) ---
	...
	* @param instanceCount The number of instances to render. Must be greater than 0.
	*/
	DrawModelInstancedEx :: proc(model: ^Model, instanceTransforms: [^]rl.Matrix, instanceColors: [^]rl.Color, instanceCount: c.int) ---
	...
	* @param instanceCount The number of instances to render. Must be greater than 0.
	*/
	DrawModelInstancedPro :: proc(model: ^Model, globalAabb: ^rl.BoundingBox, globalTransform: rl.Matrix, instanceTransforms: [^]rl.Matrix, transformsStride: c.int, instanceColors: [^]rl.Color, colorsStride: c.int, instanceCount: c.int) ---
	...
	*/
	DrawSprite :: proc(sprite: ^Sprite, position: rl.Vector3) ---
	...
	*/
	DrawSpriteEx :: proc(sprite: ^Sprite, position: rl.Vector3, size: rl.Vector2, rotation: c.float) ---
	...
	*/
	DrawSpritePro :: proc(sprite: ^Sprite, position: rl.Vector3, size: rl.Vector2, rotationAxis: rl.Vector3, rotationAngle: c.float) ---
	...
	* @param instanceCount The number of instances to render. Must be greater than 0.
	*/
	DrawSpriteInstanced :: proc(sprite: ^Sprite, instanceTransforms: [^]rl.Matrix, instanceCount: c.int) ---
	...
	* @param instanceCount The number of instances to render. Must be greater than 0.
	*/
	DrawSpriteInstancedEx :: proc(sprite: ^Sprite, instanceTransforms: [^]rl.Matrix, instanceColors: [^]rl.Color, instanceCount: c.int) ---
	...
	* @param instanceCount The number of instances to render. Must be greater than 0.
	*/
	DrawSpriteInstancedPro :: proc(sprite: ^Sprite, globalAabb: ^rl.BoundingBox, globalTransform: rl.Matrix, instanceTransforms: [^]rl.Matrix, transformsStride: c.int, instanceColors: [^]rl.Color, colorsStride: c.int, instanceCount: c.int) ---
	...
	*
	* @param system A pointer to the `ParticleSystem` to be rendered.
	...
	*/
	DrawParticleSystem :: proc(system: ^ParticleSystem, mesh: ^Mesh, material: ^Material) ---
	...
	*/
	DrawParticleSystemEx :: proc(system: ^ParticleSystem, mesh: ^Mesh, material: ^Material, transform: rl.Matrix) ---
	...
	*/
	GenMeshPoly :: proc(sides: c.int, radius: c.float, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshPlane :: proc(width: c.float, length: c.float, resX: c.int, resZ: c.int, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshCube :: proc(width: c.float, height: c.float, length: c.float, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshSphere :: proc(radius: c.float, rings: c.int, slices: c.int, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshHemiSphere :: proc(radius: c.float, rings: c.int, slices: c.int, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshCylinder :: proc(radius: c.float, height: c.float, slices: c.int, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshCone :: proc(radius: c.float, height: c.float, slices: c.int, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshTorus :: proc(radius: c.float, size: c.float, radSeg: c.int, sides: c.int, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshKnot :: proc(radius: c.float, size: c.float, radSeg: c.int, sides: c.int, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshHeightmap :: proc(heightmap: rl.Image, size: rl.Vector3, upload: c.bool) -> Mesh ---
	...
	*/
	GenMeshCubicmap :: proc(cubicmap: rl.Image, cubeSize: rl.Vector3, upload: c.bool) -> Mesh ---
	...
	*/
	UploadMesh :: proc(mesh: ^Mesh, _dynamic: c.bool) -> c.bool ---
	...
	*/
	UpdateMesh :: proc(mesh: ^Mesh) -> c.bool ---
	...
	*/
	UpdateMeshBoundingBox :: proc(mesh: ^Mesh) ---
	...
	*/
	LoadModelFromMemory :: proc(data: rawptr, size: c.uint) -> Model ---
	...
	*/
	UnloadModel :: proc(model: ^Model, unloadMaterials: c.bool) ---
	...
	*/
	UpdateModelBoundingBox :: proc(model: ^Model, updateMeshBoundingBoxes: c.bool) ---
	...
	* @return Pointer to a dynamically allocated array of ModelAnimation. NULL on failure.
	*/
	LoadModelAnimations :: proc(filePath: cstring, animCount: ^c.int, targetFrameRate: c.int) -> ^ModelAnimation ---
	...
	* @return Pointer to a dynamically allocated array of ModelAnimation. NULL on failure.
	*/
	LoadModelAnimationsFromMemory :: proc(data: rawptr, size: c.uint, animCount: ^c.int, targetFrameRate: c.int) -> ^ModelAnimation ---
	...
	*/
	GetModelAnimation :: proc(animations: ^ModelAnimation, animCount: c.int, name: cstring) -> ^ModelAnimation ---
	...
	* @param animCount Number of animations in the array.
	*/
	ListModelAnimations :: proc(animations: ^ModelAnimation, animCount: c.int) ---
	...
	*/
	SetModelImportScale :: proc(value: c.float) ---
	...
	*/
	IsLightExist :: proc(id: Light) -> c.bool ---
	...
	*/
	IsLightActive :: proc(id: Light) -> c.bool ---
	...
	*/
	SetLightActive :: proc(id: Light, active: c.bool) ---
	...
	*/
	GetLightEnergy :: proc(id: Light) -> c.float ---
	...
	*/
	SetLightEnergy :: proc(id: Light, energy: c.float) ---
	...
	*/
	GetLightSpecular :: proc(id: Light) -> c.float ---
	...
	*/
	SetLightSpecular :: proc(id: Light, specular: c.float) ---
	...
	*/
	GetLightRange :: proc(id: Light) -> c.float ---
	...
	*/
	SetLightRange :: proc(id: Light, range: c.float) ---
	...
	*/
	GetLightAttenuation :: proc(id: Light) -> c.float ---
	...
	*/
	SetLightAttenuation :: proc(id: Light, attenuation: c.float) ---
	...
	*/
	GetLightInnerCutOff :: proc(id: Light) -> c.float ---
	...
	*/
	SetLightInnerCutOff :: proc(id: Light, degrees: c.float) ---
	...
	*/
	GetLightOuterCutOff :: proc(id: Light) -> c.float ---
	...
	*/
	SetLightOuterCutOff :: proc(id: Light, degrees: c.float) ---
	...
	*/
	EnableShadow :: proc(id: Light, resolution: c.int) ---
	...
	*/
	DisableShadow :: proc(id: Light, destroyMap: c.bool) ---
	...
	*/
	IsShadowEnabled :: proc(id: Light) -> c.bool ---
	...
	*/
	HasShadowMap :: proc(id: Light) -> c.bool ---
	...
	*/
	GetShadowUpdateFrequency :: proc(id: Light) -> c.int ---
	...
	*/
	SetShadowUpdateFrequency :: proc(id: Light, msec: c.int) ---
	...
	*/
	GetShadowSoftness :: proc(id: Light) -> c.float ---
	...
	*/
	SetShadowSoftness :: proc(id: Light, softness: c.float) ---

	/**
	* @brief Gets the shadow depth bias value.
	*/
	GetShadowDepthBias :: proc(id: Light) -> c.float ---
	...
	*/
	SetShadowDepthBias :: proc(id: Light, value: c.float) ---

	/**
	* @brief Gets the shadow slope bias value.
	*/
	GetShadowSlopeBias :: proc(id: Light) -> c.float ---
	...
	*/
	SetShadowSlopeBias :: proc(id: Light, value: c.float) ---
	...
	* @return A newly initialized `ParticleSystem` structure.
	*         The caller is responsible for properly managing and freeing the system when no longer needed.
	*/
	LoadParticleSystem :: proc(maxParticles: c.int) -> ParticleSystem ---
	...
	*
	* @param system A pointer to the `ParticleSystem` to be unloaded.
	*/
	UnloadParticleSystem :: proc(system: ^ParticleSystem) ---
	...
	*
	* @param system A pointer to the `ParticleSystemCPU` where the particle will be emitted.
	...
	*/
	EmitParticle :: proc(system: ^ParticleSystem) -> c.bool ---
	...
	*
	* @param system A pointer to the `ParticleSystem` to be updated.
	* @param deltaTime The time elapsed since the last update (in seconds).
	*/
	UpdateParticleSystem :: proc(system: ^ParticleSystem, deltaTime: c.float) ---
	...
	*
	* @param system Pointer to the `ParticleSystem` to update.
	*/
	CalculateParticleSystemBoundingBox :: proc(system: ^ParticleSystem) ---
	...
	* The default billboard mode applied to the material is `BILLBOARD_Y_AXIS`.
	*
	...
	*
	* @return A `Sprite` object initialized with the given texture and frame configuration.
	*/
	LoadSprite :: proc(texture: rl.Texture2D, xFrameCount: c.int, yFrameCount: c.int) -> Sprite ---
	...
	*
	* @param sprite A pointer to the `Sprite` to be unloaded.
	*/
	UnloadSprite :: proc(sprite: ^Sprite) ---
	...
	*/
	UpdateSprite :: proc(sprite: ^Sprite, speed: c.float) ---
	...
	* @param speed The speed at which the animation progresses, in frames per second.
	*/
	UpdateSpriteEx :: proc(sprite: ^Sprite, firstFrame: c.int, lastFrame: c.int, speed: c.float) ---
	...
	*/
	LoadInterpolationCurve :: proc(capacity: c.int) -> InterpolationCurve ---
	...
	*/
	UnloadInterpolationCurve :: proc(curve: InterpolationCurve) ---
	...
	*/
	AddKeyframe :: proc(curve: ^InterpolationCurve, time: c.float, value: c.float) -> c.bool ---
	...
	*/
	EvaluateCurve :: proc(curve: InterpolationCurve, time: c.float) -> c.float ---
	...
	*/
	SetBackgroundColor :: proc(color: rl.Color) ---
	...
	*/
	SetAmbientEnergy :: proc(energy: c.float) ---
	...
	*/
	EnableSkybox :: proc(skybox: Skybox) ---
	...
	*/
	SetSkyboxRotation :: proc(pitch: c.float, yaw: c.float, roll: c.float) ---
	...
	*/
	GetSkyboxRotation :: proc() -> rl.Vector3 ---
	...
	*/
	SetSkyboxIntensity :: proc(background: c.float, ambient: c.float, reflection: c.float) ---
	...
	*/
	GetSkyboxIntensity :: proc(background: ^c.float, ambient: ^c.float, reflection: ^c.float) ---
	...
	* Default: 0.5
	*/
	SetSSAORadius :: proc(value: c.float) ---
	...
	* Default: 0.025
	*/
	SetSSAOBias :: proc(value: c.float) ---
	...
	* Default: 1
	*/
	SetSSAOIterations :: proc(value: c.int) ---
	...
	* Default: 1.0
	*/
	SetSSAOIntensity :: proc(value: c.float) ---
	...
	* Default: 1.0
	*/
	SetSSAOPower :: proc(value: c.float) ---
	...
	* Default: 0.0
	*/
	SetSSAOLightAffect :: proc(value: c.float) ---
	...
	*/
	GetSSAOLightAffect :: proc() -> c.float ---
	...
	*
	* Default: BLOOM_DISABLED
	*/
	SetBloomMode :: proc(mode: Bloom) ---
	...
	* Default: 7
	*/
	SetBloomLevels :: proc(value: c.int) ---
	...
	* Default: 0.05
	*/
	SetBloomIntensity :: proc(value: c.float) ---
	...
	*/
	GetBloomIntensity :: proc() -> c.float ---
	...
	* Default: 0
	*/
	SetBloomFilterRadius :: proc(value: c.int) ---
	...
	*/
	GetBloomFilterRadius :: proc() -> c.int ---
	...
	* Default: 0.0
	*/
	SetBloomThreshold :: proc(value: c.float) ---
	...
	*/
	GetBloomThreshold :: proc() -> c.float ---
	...
	* Default: 0.5
	*/
	SetBloomSoftThreshold :: proc(value: c.float) ---
	...
	*/
	GetBloomSoftThreshold :: proc() -> c.float ---
	...
	* Default: 64
	*/
	SetSSRMaxRaySteps :: proc(maxRaySteps: c.int) ---
	...
	* Default: 8
	*/
	SetSSRBinarySearchSteps :: proc(binarySearchSteps: c.int) ---
	...
	*/
	GetSSRBinarySearchSteps :: proc() -> c.int ---
	...
	* Default: 8.0
	*/
	SetSSRRayMarchLength :: proc(rayMarchLength: c.float) ---
	...
	*/
	GetSSRRayMarchLength :: proc() -> c.float ---
	...
	* Default: 0.2
	*/
	SetSSRDepthThickness :: proc(depthThickness: c.float) ---
	...
	*/
	GetSSRDepthThickness :: proc() -> c.float ---
	...
	* Default: 0.005
	*/
	SetSSRDepthTolerance :: proc(depthTolerance: c.float) ---
	...
	*/
	GetSSRDepthTolerance :: proc() -> c.float ---
	...
	*/
	SetSSRScreenEdgeFade :: proc(start: c.float, end: c.float) ---
	...
	*/
	GetSSRScreenEdgeFade :: proc(start: ^c.float, end: ^c.float) ---
	...
	*
	* Default: FOG_DISABLED
	*/
	SetFogMode :: proc(mode: Fog) ---
	...
	* Default: WHITE
	*/
	SetFogColor :: proc(color: rl.Color) ---
	...
	* Default: 1.0
	*/
	SetFogStart :: proc(value: c.float) ---
	...
	* Default: 0.05
	*/
	SetFogDensity :: proc(value: c.float) ---
	...
	* Default: 0.5
	*/
	SetFogSkyAffect :: proc(value: c.float) ---
	...
	*
	* Default: DOF_DISABLED
	*/
	SetDofMode :: proc(mode: Dof) ---
	...
	* Default: 10.0
	*/
	SetDofFocusPoint :: proc(value: c.float) ---
	...
	* Default: 1.0
	*/
	SetDofFocusScale :: proc(value: c.float) ---
	...
	* Default: 20.0
	*/
	SetDofMaxBlurSize :: proc(value: c.float) ---
	...
	*/
	GetDofMaxBlurSize :: proc() -> c.float ---
	...
	* Default: false
	*/
	SetDofDebugMode :: proc(enabled: c.bool) ---
	...
	*/
	SetTonemapMode :: proc(mode: Tonemap) ---
	...
	* Default: 1.0
	*/
	SetTonemapExposure :: proc(value: c.float) ---
	...
	*/
	GetTonemapExposure :: proc() -> c.float ---
	...
	* Default: 1.0
	*/
	SetTonemapWhite :: proc(value: c.float) ---
	...
	* Default: 1.0
	*/
	SetBrightness :: proc(value: c.float) ---
	...
	* Default: 1.0
	*/
	SetContrast :: proc(value: c.float) ---
	...
	* Default: 1.0
	*/
	SetSaturation :: proc(value: c.float) ---
	...
	*/
	LoadSkybox :: proc(filePath: cstring, layout: rl.CubemapLayout) -> Skybox ---
	...
	* @return The loaded skybox object.
	*/
	LoadSkyboxFromMemory :: proc(image: rl.Image, layout: rl.CubemapLayout) -> Skybox ---
	...
	* @return The loaded skybox object.
	*/
	LoadSkyboxPanorama :: proc(filePath: cstring, size: c.int) -> Skybox ---
	...
	* @return The loaded skybox object.
	*/
	LoadSkyboxPanoramaFromMemory :: proc(image: rl.Image, size: c.int) -> Skybox ---
	...
	*
	* @see IsPointInFrustumBoundingBox()
	*/
	IsPointInFrustum :: proc(position: rl.Vector3) -> c.bool ---
	...
	*
	* @see IsSphereInFrustumBoundingBox()
	*/
	IsSphereInFrustum :: proc(position: rl.Vector3, radius: c.float) -> c.bool ---
	...
	*
	* @see IsAABBInFrustumBoundingBox()
	*/
	IsAABBInFrustum :: proc(aabb: rl.BoundingBox) -> c.bool ---
	...
	* @see IsAABBInFrustum()
	*/
	IsOBBInFrustum :: proc(aabb: rl.BoundingBox, transform: rl.Matrix) -> c.bool ---
	...
	*/
	IsPointInFrustumBoundingBox :: proc(position: rl.Vector3) -> c.bool ---
	...
	*/
	IsSphereInFrustumBoundingBox :: proc(position: rl.Vector3, radius: c.float) -> c.bool ---
	...
	* @see IsAABBInFrustum()
	*/
	IsAABBInFrustumBoundingBox :: proc(aabb: rl.BoundingBox) -> c.bool ---
	...
	*/
	GetBufferColor :: proc() -> rl.Texture2D ---
	...
	*/
	DrawBufferAlbedo :: proc(x: c.float, y: c.float, w: c.float, h: c.float) ---
	...
	* @param h Height of the drawn buffer.
	*/
	DrawBufferEmission :: proc(x: c.float, y: c.float, w: c.float, h: c.float) ---
	...
	* @param h Height of the drawn buffer.
	*/
	DrawBufferNormal :: proc(x: c.float, y: c.float, w: c.float, h: c.float) ---
	...
	* @param h Height of the drawn buffer.
	*/
	DrawBufferORM :: proc(x: c.float, y: c.float, w: c.float, h: c.float) ---
	...
	* @param h Height of the drawn buffer.
	*/
	DrawBufferSSAO :: proc(x: c.float, y: c.float, w: c.float, h: c.float) ---
	...
	* @param h Height of the drawn buffer.
	*/
	DrawBufferBloom :: proc(x: c.float, y: c.float, w: c.float, h: c.float) ---
}
